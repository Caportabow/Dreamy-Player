...
<template>
  <button
    type="button"
    class="group flex items-center gap-2.5 rounded-pillow-sm text-left transition-all duration-300 w-full px-3 py-2.5 hover:translate-x-0.5 hover:bg-white/5"
    :class="active ? 'bg-white/7 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06),0_0_30px_-8px_rgba(140,110,200,0.4)]' : ''"
    :aria-label="`Open your profile — ${auth.displayName}`"
    @click="go"
  >
    <!-- Avatar: uploaded picture, or initials on a soft gradient -->
    <span
      class="relative shrink-0 overflow-hidden rounded-full border border-white/8 shadow-inner-soft h-10 w-10"
    >
      <img
        v-if="avatarUrl"
        :src="avatarUrl"
    ...
    <span class="min-w-0 flex-1">
      <span class="block truncate font-medium text-cream text-sm">
        {{ auth.displayName }}
      </span>
      <span class="block truncate text-cream-dim text-xs">
        @{{ auth.user?.username }}
      </span>
    </span>
  </button>
